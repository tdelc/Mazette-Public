...
# Fabrique un jeton valable jusqu'à `expire_le`.
jeton_auth <- function(mot_de_passe, expire_le) {
  expire <- as.character(as.integer(as.numeric(as.POSIXct(expire_le))))
  paste0(expire, ".",
         as.character(openssl::sha256(expire, key = mot_de_passe)))
}

# Vérifie un jeton contre les mots de passe actuellement valides.
# L'expiration est contrôlée ICI, côté serveur : la durée de vie du cookie
# est fixée par le navigateur, donc par le client, on ne peut pas s'y fier.
jeton_valide <- function(jeton, mots_de_passe, maintenant = Sys.time()) {
  if (is.null(jeton) || length(jeton) != 1 || !nzchar(jeton)) return(FALSE)
  if (length(mots_de_passe) == 0) return(FALSE)

  morceaux <- strsplit(jeton, ".", fixed = TRUE)[[1]]
  if (length(morceaux) != 2) return(FALSE)

  expire <- suppressWarnings(as.numeric(morceaux[1]))
  if (is.na(expire) || expire <= as.numeric(maintenant)) return(FALSE)

  attendu <- vapply(mots_de_passe,
                    function(mp) as.character(openssl::sha256(morceaux[1], key = mp)),
                    character(1), USE.NAMES = FALSE)
  isTRUE(any(tolower(morceaux[2]) == tolower(attendu)))
}

# Petite couche JavaScript : lecture au chargement, pose et effacement à la
# demande. Le cookie est posé depuis le navigateur, il ne peut donc pas être
# HttpOnly — acceptable ici, mais c'est la limite de l'approche.
script_cookie_auth <- function(nom = NOM_COOKIE_AUTH) {
  tags$script(HTML(sprintf("
function mzLireCookie(nom) {
  var p = document.cookie.split('; ').find(function(c) {
    return c.indexOf(nom + '=') === 0;
  });
  return p ? decodeURIComponent(p.substring(nom.length + 1)) : '';
}
function mzPoserCookie(valeur, secondes) {
  var sur = location.protocol === 'https:' ? '; Secure' : '';
  document.cookie = '%s=' + encodeURIComponent(valeur) +
    '; max-age=' + secondes + '; path=/; SameSite=Lax' + sur;
}
function mzEffacerCookie() {
  document.cookie = '%s=; max-age=0; path=/; SameSite=Lax';
}
$(document).on('shiny:connected', function() {
  Shiny.setInputValue('cookie_auth', mzLireCookie('%s'), {priority: 'event'});
});
", nom, nom, nom)))
}
